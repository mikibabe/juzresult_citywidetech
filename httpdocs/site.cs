...
bool AppySiteSettings()
{
	...
	{
		m_sSalesEmail = GetSiteSettings("sales_email", "customer@pixellive.co.nz");
		if(g_bDemo)
			m_sCompanyTitle = Capital(GetSiteSettings("company_name", "Wholesale NZ Limited"));
		else
			m_sCompanyTitle = GetSiteSettings("company_name", "Pixel Live Ltd.");
